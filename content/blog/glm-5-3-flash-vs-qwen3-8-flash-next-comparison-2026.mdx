...
†Qwen3.8-Flash-Next ships as open weights and is not listed on [Qwen Cloud](https://www.qwencloud.com/), which sells the separate `qwen3.8-flash` model. Self-hosting cost depends on your own hardware.
...
The [model card](https://huggingface.co/zai-org/GLM-5.3-Flash) states it "outperforms GLM-5.2 across benchmarks and real-world workloads at one-tenth the price, while approaching Claude Opus 4.8 on coding and agentic benchmarks." The price comparison is against GLM-5.2, Z.ai's own previous generation — not against Claude Opus.
...
Z.ai publishes list prices per 1M tokens ([pricing page](https://docs.z.ai/guides/overview/pricing)):
